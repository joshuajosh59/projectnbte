  ...
  <div id="institution">
    <b-row>
      <div class="top-display">
        <b style="margin-left: 10px; font-size: 20px" class="montserrat">Add Webometric</b>
      </div>
    </b-row>
    <br>
    <div style="margin-top: 20px;color: #333333; font-size: 15px; font-family: montserrat">
      <b-form @submit.prevent="addWebometric">
        <b-form-group label="Name" label-for="name" label-align="center">
          <b-form-input
            ...
            v-model="name"
            required
            placeholder="Enter Webometric Name"
          />
        </b-form-group>
        <b-form-group label="Weigth" label-for="weigth" label-align="center">
          <b-form-input
            ...
            v-model="weigth"
            required
            placeholder="Enter Webometric Weigth"
          />
        </b-form-group>
        <b-row>
          <b-col md="4"></b-col>
...
export default {
  ...
  methods: {
    addWebometric() {
      ...
      this.$http.put(url + `webometrics/${this.id}`, { name, 'weight': weigth }, { headers: getHeader() }).then((response) => {
        if (response.data.success) {
          this.$swal({
            type: 'success',
            title: 'Sucess',
            text: 'Webometric updated successfully',
            timer: 2000,
          });
        }
      })
    ...
    getValues() {
      this.$http.get(url + `webometrics/${this.id}`, { headers: getHeader() }).then((response) => {
